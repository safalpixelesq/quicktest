...
export async function getStaticProps({ params }) {
	...
	// loop through list nad for each generate a json file
	await list
		.map(async (item) => {
			const fullslug = path.join(contentDir, `${item.slug}.json`);
			const fileContents = await fs.readFile(fullslug, "utf8");
			const blogContent = await JSON.parse(fileContents);
			const meta = await {
				apimode: "component",
				...
				content: [
					{
						...
						subtitle: new Date(item.pub_date)
							.toDateString()
							.toUpperCase(),
						title: await item.title,
			...
			await fs.writeFile(
				`./content/pages/blog/${item.slug}.json`,
