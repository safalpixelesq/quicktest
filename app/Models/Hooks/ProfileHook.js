...
ProfileHook.setAvatar = async (profileInstance) => {
	if (!profileInstance.avatar || profileInstance.avatar === "") {
		...
		// We can also have local upload method
...
ProfileHook.setAvatars = async (profileArray) => {
	...
	profileArray.map((profileInstance) => {
		if (!profileInstance.avatar || profileInstance.avatar === "") {
			...
			profileInstance.avatar =
				Env.get("APP_URL", "") + profileInstance.avatar;
		}
	});
};
...
ProfileHook.setDefaults = async (profileInstance) => {
	...
	const avatarUrl = await axios
		.get(
			`http://picasaweb.google.com/data/entry/api/user/${user.email}?alt=json`
		)
		.then((res) => {
			...
			if (
				...
				res.data &&
				res.data.entry &&
				res.data.entry.gphoto$thumbnail &&
				res.data.entry.gphoto$thumbnail.$t
			) {
				return Promise.resolve(res.data.entry.gphoto$thumbnail.$t);
			}
			return Promise.resolve(
				gravatar.url(user.email, { s: "100", r: "x", d: "retro" }, true)
			);
		})
		.catch(() =>
			Promise.resolve(
